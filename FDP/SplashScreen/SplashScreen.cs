...
namespace SplashScreen
{
	...
	public partial class SplashScreen : Form
	{
		...
		public const int TIMER_INTERVAL = 10; //50
		...
		public SplashScreen()
		{
            ...
            labelloading.Left = this.ClientSize.Width / 2 - labelloading.Width / 2;
            labelloading.Top = this.Height / 2 + pictureBox1.Height / 2 + labelloading.Height / 2 + 10;
            labelLoading2.Left = this.ClientSize.Width / 2 - labelLoading2.Width / 2;
            labelLoading2.Top = this.Height / 2 + pictureBox1.Height / 2 + labelloading.Height / 2 + labelLoading2.Height / 2 + 20;
             */
        }
        ...
        static public void ShowSplashScreen(int x, int y, int w, int h)
        {
            // Make sure it's only launched once.
            if (ms_frmSplash != null)
                return;
            ...
            //ms_oThread = new Thread(new ThreadStart(SplashScreen.ShowForm));
            ms_oThread = new Thread(() => SplashScreen.ShowForm(x, y, w, h));
            ms_oThread.IsBackground = true;
            ms_oThread.SetApartmentState(ApartmentState.STA);
            ms_oThread.Start();
            while (ms_frmSplash == null || ms_frmSplash.IsHandleCreated == false)
        ...
        static private void ShowForm(int x, int y, int w, int h)
        {
            ms_frmSplash = new SplashScreen();
            ms_frmSplash.WindowState = FormWindowState.Normal;
            ms_frmSplash.StartPosition = FormStartPosition.Manual;
            ms_frmSplash.Left = x;
            ms_frmSplash.Top = y;
            ms_frmSplash.Width = w;
            ms_frmSplash.Height = h;

            //ms_frmSplash.ClientSize = ms_frmSplash.BackgroundImage.Size;
            ms_frmSplash.ClientSize = new Size(w, h);
            ms_frmSplash.pictureBox1.Left = ms_frmSplash.ClientSize.Width / 2 - ms_frmSplash.pictureBox1.Width / 2;
            ms_frmSplash.pictureBox1.Top = ms_frmSplash.Height / 2 - ms_frmSplash.pictureBox1.Height / 2;
            ms_frmSplash.labelloading.Left = ms_frmSplash.ClientSize.Width / 2 - ms_frmSplash.labelloading.Width / 2;
            ms_frmSplash.labelloading.Top = ms_frmSplash.Height / 2 + ms_frmSplash.pictureBox1.Height / 2 + ms_frmSplash.labelloading.Height / 2 + 10;
            ms_frmSplash.labelLoading2.Left = ms_frmSplash.ClientSize.Width / 2 - ms_frmSplash.labelLoading2.Width / 2;
            ms_frmSplash.labelLoading2.Top = ms_frmSplash.Height / 2 + ms_frmSplash.pictureBox1.Height / 2 + ms_frmSplash.labelloading.Height / 2 + ms_frmSplash.labelLoading2.Height / 2 + 20;
