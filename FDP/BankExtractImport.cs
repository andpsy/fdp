...
namespace FDP
{
    public partial class BankExtractImport : UserForm
    {
        ...
        public BankExtractImport()
        {
            base.Maximized = FormWindowState.Maximized;
            InitializeComponent();
            checkedComboBoxProperties.ItemCheck += new System.Windows.Forms.ItemCheckEventHandler(this.checkedComboBoxProperties_ItemCheck);
        }
        ...
        private void FillCombos()
        {
            ...
            if (dtc1 != null)
            {
                foreach (DataRow dre in dtc1.Rows)
                {
                    toolStripComboBoxOwner.Items.Add(dre["name"]);
                }
            }
        }

        private void FillPredictedIncomeExpenses(string owner_name)
        {
            try
            {
                object owner_id = (new DataAccess(CommandType.StoredProcedure, "OWNERSsp_get_id_by_name", new object[] { new MySqlParameter("_NAME", owner_name) })).ExecuteScalarQuery();
                OwnerId = owner_id;
                FillPredictedIncomeExpenses(Convert.ToInt32(owner_id));
            }
            catch (Exception exp) { throw exp; }
        }

        private void FillPredictedIncomeExpenses(int owner_id)
        {
            try
            {
                DataAccess da = new DataAccess(CommandType.StoredProcedure, "INCOME_EXPENSESsp_get_by_owner_id2", new object[] { new MySqlParameter("_OWNER_ID", owner_id) });
                ...
                //dataGrid1.dataGridView.DataSource = predictedIESource;
                /*
        ...
        private void dataGridViewPredictedIE_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            dataGrid1.dataGridView["IMPORT", e.RowIndex].Style.BackColor = Color.LightPink;
            dataGrid1.dataGridView["AMOUNT", e.RowIndex].Style.BackColor = Color.LightPink;
            dataGrid1.dataGridView["AMOUNT", e.RowIndex].Value = dataGrid1.dataGridView["BALLANCE", e.RowIndex].Value;
            dataGrid1.dataGridView["AMOUNT", e.RowIndex].ReadOnly = false;
            dataGrid1.dataGridView["AMOUNT_EXCHANGED", e.RowIndex].ReadOnly = true;
        }
                ...
                if (toolStripTextBoxFile.Text != "") // && toolStripTextBoxSheet.Text != "")
                {
                    ...
                    */
                    ...
                    if (!file_loaded_before)
                    {
                        ...
                        //if (dataSetFromCSV != null && dataSetFromCSV.Tables.Count > 0 && dataSetFromCSV.Tables[0].Rows.Count > 0 && dataSetFromCSV.Tables[0].Columns[0].ColumnName.ToLower() == "titlu cont")
                        if (dataSetFromCSV != null && dataSetFromCSV.Tables.Count > 0 && dataSetFromCSV.Tables[0].Rows.Count > 0 && dataSetFromCSV.Tables[0].Columns[0].ColumnName.ToLower() == comboBoxAccountTitle.SelectedItem.ToString().ToLower())
                        {
                            ...
                            //dataSetFromCSV.Tables[0].Columns.Remove("cont");
                    ...
                    /*
                    DataGridViewCheckBoxColumn dgvcbc = new DataGridViewCheckBoxColumn();
                    dgvcbc.Name = "IMPORTED";
                    dgvcbc.DataPropertyName = "IMPORTED";
                    dataGridView.Columns.Add(dgvcbc);
                    */
                    //dataGridView.Columns.Remove("IMPORTED");
                    ...
                    */
                    ...
                    try
                    {
                        SelectOwner(BankAccountSequence);
                    }
                    catch(Exception exp)
                    {
                        base.ShowErrorsDialog(Language.GetMessageBoxText("errorGettingOwnerFromAccountSequence", String.Format("There was an error getting the Owner information based on the Account sequence from the Excel file: {0}", exp.Message)));
                        LogWriter.Log(exp, SettingsClass.ErrorLogFile);
                    }
                    try
                    {
                        FillPredictedIncomeExpenses(toolStripComboBoxOwner.SelectedItem.ToString());
                    }
                    catch (Exception exp)
                    {
                        base.ShowErrorsDialog(Language.GetMessageBoxText("errorGettingOwnerIE", String.Format("There was an error getting the Owner (Predicted) I/E's: {0}", exp.Message)));
                        LogWriter.Log(exp, SettingsClass.ErrorLogFile);
                    }
                }
            }
        ...
        private void SelectOwner(string bank_account_sequence)
        {
            try
            {
                ...
                BankAccount = (new DataAccess(CommandType.StoredProcedure, "OWNERSsp_get_account_by_sequence", new object[] { new MySqlParameter("_BANK_ACCOUNT_SEQUENCE", bank_account_sequence) })).ExecuteScalarQuery().ToString();
        ...
        private void FillProperties(string owner_name)
        {
            DataTable dt = (new DataAccess(CommandType.StoredProcedure, "PROPERTIESsp_get_by_owner_name", new object[] { new MySqlParameter("_OWNER_NAME", owner_name) })).ExecuteSelectQuery().Tables[0];
            ...
            toolStripComboBoxProperty.Items.Add("Not property related");
            ...
            checkedComboBoxProperties.MaxDropDownItems = 5;
            // Make the "Name" property the one to display, rather than the ToString() representation.
            checkedComboBoxProperties.DisplayMember = "Name";
            checkedComboBoxProperties.ValueSeparator = ", ";
            // Check the first 2 items.
            //checkedComboBoxProperties.SetItemChecked(0, true);
            //checkedComboBoxProperties.SetItemChecked(1, true);
        ...
        private void toolStripButtonImport_Click(object sender, EventArgs e)
        {
            if (!checkBoxManual.Checked)
            {
                ...
                foreach (DataGridViewRow dgvr in dataGrid1.dataGridView.Rows)
                {
                    ...
                    {
                        errorProvider1.SetError(dataGrid1.dataGridView, "");
                        ...
                        {
                            errorProvider1.SetError(dataGrid1.dataGridView, Language.GetErrorText("errorMultipleInvoices", "You have selected multiple invoice! Please select only one invoice!"));
                            base.ErrorList.Add(new KeyValuePair<string, string>(dataGrid1.dataGridView.Name, Language.GetErrorText("errorMultipleInvoices", "You have selected multiple invoice! Please select only one invoice!")));
                ...
                */
                ...
                try
                {
                    invoice_id = Convert.ToInt32(dataGrid1.dataGridView.SelectedRows[0].Cells["INVOICE_ID"].Value);
                }
                catch { invoice_id = -1; }
                ...
                {
                    DialogResult ans = MessageBox.Show(String.Format(Language.GetMessageBoxText("AmountsDontMatch", "The selected value from the file ({0}) and the sum of the selected I/E(s) ({1}) don't match! Continue?"), Math.Abs(Convert.ToDouble(dataGridView["SUMA", dataGridView.CurrentRow.Index].Value)).ToString(), total_amount.ToString()), "", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                ...
                if (doit)
                {
                    ...
                    foreach (DataGridViewRow dgvr in dataGrid1.dataGridView.Rows)
                    {
                        ...
                        if (Convert.ToBoolean(dgvr.Cells["IMPORT"].Value == DBNull.Value ? false : dgvr.Cells["IMPORT"].Value))
                        {
                            ...
                            #region --- old ---
                            //Check if Predicted IE has invoice and update invoice status accordingly (paid or partial paid) and ballance
                            /*
                            try
                            {
                                IncomeExpensesClass.InsertIE(
                                    false,
                                    dgvr.Cells["TYPE"].Value.ToString().ToUpper() == "EXPENSE" ? false : true,
                                    true, // for bank
                                    dgvr.Cells["CURRENCY"].Value,
                                    //dgvr.Cells["AMOUNT"].Value,
                                    amount,
                                    //Convert.ToDateTime(dataGridView["data tranzactiei", dataGridView.CurrentRow.Index].Value),
                                    data,
                                    dgvr.Cells["OWNER_ID"].Value,
                                    dgvr.Cells["PROPERTY_ID"].Value,
                                    dgvr.Cells["CONTRACTSERVICE_ID"].Value,
                                    dgvr.Cells["SERVICE_DESCRIPTION"].Value,
                                    dgvr.Cells["MONTH"].Value,
                                    dgvr.Cells["INVOICEREQUIREMENT_ID"].Value,
                                    dgvr.Cells["INVOICE_ID"].Value,
                                    dgvr.Cells["CONTRACT_SERVICE_ADDITIONAL_COST_ID"].Value,
                                    DBNull.Value
                                    );
                            }
                            catch (Exception exp) { LogWriter.Log(exp, SettingsClass.ErrorLogFile); }
                            ...
                            try
                            {
                                //string SP = String.Format("UPDATE OWNERS SET {0} = IFNULL({1},0) + {2} WHERE ID = {3};", AccountField, AccountField, dgvr.Cells["AMOUNT"].Value.ToString(), dgvr.Cells["OWNER_ID"].Value.ToString());
                                string SP = String.Format("UPDATE OWNERS SET {0} = IFNULL({1},0) - {2} WHERE ID = {3};", AccountField, AccountField, amount.ToString(), dgvr.Cells["OWNER_ID"].Value.ToString());
                    ...
                    FillPredictedIncomeExpenses(Convert.ToInt32(OwnerId));
                }
            }
            else
            {
                ...
                try
                {
                    ...
                    foreach (CCBoxItem ci in checkedComboBoxProperties.CheckedItems)
                    {
                        ...
                        //PropertyId = property.Trim().ToLower() == "not property related"? null : (new DataAccess(CommandType.StoredProcedure, "PROPERTIESsp_get_id_by_name", new object[] { new MySqlParameter("_NAME", property) })).ExecuteScalarQuery().ToString();
        ...
        private bool ValidateData()
        {
            ...
            //errorProvider1.SetError(userTextBoxManualExpenseAmount, "");
            errorProvider1.SetError(userTextBoxTotal, "");
            if (toolStripComboBoxOwner.SelectedItem == null || toolStripComboBoxOwner.SelectedItem.ToString().Trim() == "")
            {
                errorProvider1.SetError(toolStrip1, Language.GetErrorText("errorEmptyOwner", "You must select the owner!"));
                base.ErrorList.Add(new KeyValuePair<string, string>(toolStrip1.Name, Language.GetErrorText("errorEmptyOwner", "You must select the owner!")));
                toReturn = false;
            }
            /*
            if (toolStripComboBoxProperty.SelectedItem == null || toolStripComboBoxProperty.SelectedItem.ToString().Trim() == "")
            {
                errorProvider1.SetError(toolStrip1, Language.GetErrorText("errorEmptyProperty", "You must select the property!"));
                base.ErrorList.Add(new KeyValuePair<string, string>(toolStrip1.Name, Language.GetErrorText("errorEmptyProperty", "You must select the property!")));
                toReturn = false;
            }
            */
            if (checkedComboBoxProperties.CheckedItems.Count == 0)
            {
                errorProvider1.SetError(checkedComboBoxProperties, Language.GetErrorText("errorEmptyProperty", "You must select the property!"));
                base.ErrorList.Add(new KeyValuePair<string, string>(checkedComboBoxProperties.Name, Language.GetErrorText("errorEmptyProperty", "You must select the property!")));
                toReturn = false;
            }
        ...
        private void checkedComboBoxProperties_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            CCBoxItem item = checkedComboBoxProperties.Items[e.Index] as CCBoxItem;
