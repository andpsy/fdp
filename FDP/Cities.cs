...
namespace FDP
{
    public partial class Cities : UserForm
    {
        ...
        public void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            DataRow city = ((DataTable)((BindingSource)dataGrid1.dataGridView.DataSource).DataSource).NewRow();
            var f = new DynamicFormLists("cities", city, 0);
            EditMode = 1; // ADD
            main m = FindMainForm();
            f.Launcher = this;
        ...
        public void SaveAddRecord()
        {
            ...
            if (f.DialogResult == DialogResult.OK)
            {
                try
                {
                    DataRow city = f.return_data_row;
                    object[] mySqlParams = dataGrid1.da.GenerateMySqlParameters(city.Table, city.ItemArray, 0);
                    ((DataTable)dataGrid1.da.bindingSource.DataSource).Rows.Add(city);
        ...
        public void buttonEdit_Click(object sender, EventArgs e)
        {
            SplashScreen.SplashScreen.ShowSplashScreen(((Panel)this.FindForm().Parent).PointToScreen(Point.Empty).X, ((Panel)this.FindForm().Parent).PointToScreen(Point.Empty).Y, ((Panel)this.FindForm().Parent).Width, ((Panel)this.FindForm().Parent).Height);
            Application.DoEvents();
            DataRow city = ((DataRowView)dataGrid1.dataGridView.SelectedRows[0].DataBoundItem).Row;
            var f = new DynamicFormLists("cities", city, 1);
            EditMode = 2; // EDIT
            main m = FindMainForm();
            f.Launcher = this;
            this.ChildLaunched = f;
        ...
        public void SaveEditRecord()
        {
            ...
            if (f.DialogResult == DialogResult.OK)
            {
                ...
                {
                    DataRow city = f.return_data_row;
                    object[] mySqlParams = dataGrid1.da.GenerateMySqlParameters(city.Table, city.ItemArray, 1);
