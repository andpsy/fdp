...
namespace FDP
{
    public partial class ContractReasons : UserForm
    {
        public DataSet Content = new DataSet();
        public bool Selectable = false;

        public ContractReasons()
        {
        ...
        public ContractReasons(bool selectable)
        {
            base.Maximized = FormWindowState.Maximized;
        ...
        public void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            DataRow reason = ((DataTable)((BindingSource)dataGrid1.dataGridView.DataSource).DataSource).NewRow();
            var f = new DynamicFormLists("contractreasons", reason, 0);
            EditMode = 1; // ADD
            main m = FindMainForm();
            f.Launcher = this;
        ...
        public void SaveAddRecord()
        {
            ...
            if (f.DialogResult == DialogResult.OK)
            {
                try
                {
                    DataRow reason = f.return_data_row;
                    object[] mySqlParams = dataGrid1.da.GenerateMySqlParameters(reason.Table, reason.ItemArray, 0);
                    ((DataTable)dataGrid1.da.bindingSource.DataSource).Rows.Add(reason);
        ...
        public void buttonEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                DataRow reason = ((DataRowView)dataGrid1.dataGridView.SelectedRows[0].DataBoundItem).Row;
                var f = new DynamicFormLists("contractreasons", reason, 1);
                EditMode = 2; // EDIT
                main m = FindMainForm();
                f.Launcher = this;
                this.ChildLaunched = f;
        ...
        public void SaveEditRecord()
        {
            ...
            if (f.DialogResult == DialogResult.OK)
            {
                ...
                {
                    DataRow reason = f.return_data_row;
                    object[] mySqlParams = dataGrid1.da.GenerateMySqlParameters(reason.Table, reason.ItemArray, 1);
