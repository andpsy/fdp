...
namespace FDP
{
    public partial class Districts : UserForm
    {
        ...
        public Districts(bool selectable)
        {
            base.Maximized = FormWindowState.Maximized;
        ...
        public void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            DataRow district = ((DataTable)((BindingSource)dataGrid1.dataGridView.DataSource).DataSource).NewRow();
            var f = new DynamicFormLists("districts", district, 0);
            EditMode = 1; // ADD
            main m = FindMainForm();
            f.Launcher = this;
        ...
        public void SaveAddRecord()
        {
            ...
            if (f.DialogResult == DialogResult.OK)
            {
                try
                {
                    DataRow district = f.return_data_row;
                    object[] mySqlParams = dataGrid1.da.GenerateMySqlParameters(district.Table, district.ItemArray, 0);
                    ((DataTable)dataGrid1.da.bindingSource.DataSource).Rows.Add(district);
        ...
        public void buttonEdit_Click(object sender, EventArgs e)
        {
            SplashScreen.SplashScreen.ShowSplashScreen(((Panel)this.FindForm().Parent).PointToScreen(Point.Empty).X, ((Panel)this.FindForm().Parent).PointToScreen(Point.Empty).Y, ((Panel)this.FindForm().Parent).Width, ((Panel)this.FindForm().Parent).Height);
            Application.DoEvents();
            DataRow district = ((DataRowView)dataGrid1.dataGridView.SelectedRows[0].DataBoundItem).Row;
            var f = new DynamicFormLists("districts", district, 1);
            EditMode = 2; // EDIT
            main m = FindMainForm();
            f.Launcher = this;
            this.ChildLaunched = f;
        ...
        public void SaveEditRecord()
        {
            ...
            if (f.DialogResult == DialogResult.OK)
            {
                ...
                {
                    DataRow district = f.return_data_row;
                    object[] mySqlParams = dataGrid1.da.GenerateMySqlParameters(district.Table, district.ItemArray, 1);
