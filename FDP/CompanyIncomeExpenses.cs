...
namespace FDP
{
    public partial class CompanyIncomeExpenses : UserForm
    {
        ...
        public CompanyIncomeExpenses(DataRow dr)
        {
            base.Maximized = FormWindowState.Maximized;
            base.CheckDataOnClosing = true;
            //NewDR = dr;
        ...
        {
            DataRow dr = (new DataAccess(CommandType.StoredProcedure, "COMPANY_INCOME_EXPENSESsp_get_by_id2", new object[] { new MySqlParameter("_ID", id) })).ExecuteSelectQuery().Tables[0].Rows[0];
            PopulateRow(dr);
        }

        private void CompanyIncomeExpenses_Load(object sender, EventArgs e)
        {
        ...
        public void GenerateMySqlParameters()
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        MySqlParameter _PROPERTY_ID = new MySqlParameter("_PROPERTY_ID", DBNull.Value);
                        MySqlParameters.Add(_PROPERTY_ID);
                    }
                    ...
                    switch (comboBoxPeriod.SelectedItem.ToString())
                    {
                        case "Day":
                            ...
                            for (int i = 0; i < days; i++)
                            {
                                ...
                                DataAccess da = new DataAccess(CommandType.StoredProcedure, "COMPANY_INCOME_EXPENSESsp_insert", MySqlParameters.ToArray());
        ...
        private bool ValidateData()
        {
            ...
            if ((comboBoxOwner.SelectedValue == null || comboBoxOwner.SelectedIndex < 1) && !checkBoxNotOwnerRelated.Checked)
            {
                errorProvider1.SetError(comboBoxOwner, Language.GetErrorText("errorEmptyOwner", "You must select the owner or check the \"Not owner related\" option!"));
                base.ErrorList.Add(new KeyValuePair<string, string>(comboBoxOwner.Name, Language.GetErrorText("errorEmptyOwner", "You must select the owner or check the \"Not owner related\" option!")));
                toReturn = false;
            }
            ...
            if (dataGridViewProperties.SelectedRows.Count <= 0 && !checkBoxNotOwnerRelated.Checked)
            {
                errorProvider1.SetError(dataGridViewProperties, Language.GetErrorText("errorEmptyProperty", "You must select the property or check the \"Not owner related\" option!"));
                base.ErrorList.Add(new KeyValuePair<string, string>(dataGridViewProperties.Name, Language.GetErrorText("errorEmptyProperty", "You must select the property or check the \"Not owner related\" option!")));
                toReturn = false;
            }
        ...
        private void comboBoxOwner_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    checkBoxNotOwnerRelated.Checked = false;
                ...
                {
                    checkBoxNotOwnerRelated.Checked = true;
        ...
        private void FillPayments(int company_income_expense_id)
        {
            DataAccess da = new DataAccess(CommandType.StoredProcedure, "COMPANY_INCOME_EXPENSESsp_get_payments", new object[] { new MySqlParameter("_COMPANY_INCOME_EXPENSE_ID", company_income_expense_id) });
