    ...
    public partial class main : UserForm
    {
        ...
        private void main_Load(object sender, EventArgs e)
        {
            ...
            */
            ...
            foreach(string x in comboBoxLanguageFiles.Items){
                if (ConfigurationManager.AppSettings["language"].ToString() == x)
                {
                    comboBoxLanguageFiles.SelectedItem = x;
                    break;
                }
            }

            fontDialog1.ShowColor = false;
            userTextBoxFontFamily.Text = ConfigurationManager.AppSettings["FontTheme"];
        }
        ...
        private void buttonBackupDB_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                object ret = ExecuteBatchFile(SettingsClass.MySqlToolsRestoreBatchFile, new string[] { String.Format("\"{0}\" --no-data --add-drop-table -B {1} -u {2} --password={3} --routines --triggers > \"{4}\"", mysqldump_path, SettingsClass.ConnectionStringDataBase, SettingsClass.ConnectionStringUser, SettingsClass.ConnectionStringPassword, userTextBoxBackupFile.Text.Trim()) });
        ...
        private void pictureBoxFontSelect_Click(object sender, EventArgs e)
        {
            if (fontDialog1.ShowDialog() != DialogResult.Cancel)
            {
                userTextBoxFontFamily.Text = fontDialog1.Font.FontFamily.Name;
            }
        }

        private void buttonSaveFontTheme_Click(object sender, EventArgs e)
        {
            if (userTextBoxFontFamily.Text.Trim() != "")
            {
                try
                {
                    System.Configuration.Configuration config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
                    config.AppSettings.Settings.Remove("FontTheme");
                    config.AppSettings.Settings.Add("FontTheme", userTextBoxFontFamily.Text);
                    config.Save(ConfigurationSaveMode.Modified, true);
                    MessageBox.Show("You must close and reopen the application to apply the changes!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception exp)
        ...
        {
            if (comboBoxLanguageFiles.SelectedIndex > -1)
            {
                try
                {
                    System.Configuration.Configuration config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
                    config.AppSettings.Settings.Remove("language");
                    config.AppSettings.Settings.Add("language", comboBoxLanguageFiles.SelectedItem.ToString());
                    config.Save(ConfigurationSaveMode.Modified, true);
                    MessageBox.Show("You must close and reopen the application to apply the changes!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception exp)
